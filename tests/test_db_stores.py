...
class SQLiteStoresTests(unittest.TestCase):
    ...
    def test_awg_store_strips_raw_script_output_and_keeps_only_vpn_key(self) -> None:
        profile_state = SQLiteProfileStateStore(self.db)
        profile_state.write(
            {
                "alice": {"type": "none", "created_at": "2026-01-01T00:00", "updated_at": "2026-01-01T00:00"},
        ...
        store.write(
            {
                "alice": {"servers": {"de": {"config": raw_output, "wg_conf": "[Interface]\nPrivateKey = hidden\n\n[Peer]\nPublicKey = peer\n", "created_at": "2026-01-01"}}},
            }
        )
        rows = store.read()
        self.assertEqual(rows["alice"]["servers"]["de"]["config"], "vpn://awg-secret-token")
        with self.db.connect() as conn:
            stored = conn.execute(
                "SELECT config_text FROM awg_server_configs WHERE profile_name = ? AND server_key = ?",
                ("alice", "de"),
            ).fetchone()
        self.assertEqual(str(stored["config_text"]), "vpn://awg-secret-token")

    def test_awg_schema_migration_scrubs_existing_raw_output(self) -> None:
        legacy_db = SQLiteDB(f"{self.tmpdir.name}/legacy/bot.sqlite3")
        with legacy_db.transaction() as conn:
            conn.execute(
                """
                CREATE TABLE awg_server_configs (
            ...
            ensure_schema(conn)
            row = conn.execute(
                "SELECT config_text, wg_conf FROM awg_server_configs WHERE profile_name = ? AND server_key = ?",
                ("alice", "de"),
            ).fetchone()
        self.assertEqual(str(row["config_text"]), "vpn://legacy-token")
        self.assertIn("[Interface]", str(row["wg_conf"]))

    def test_sqlite_db_hardens_file_and_directory_permissions(self) -> None:
        db = SQLiteDB(f"{self.tmpdir.name}/secure/bot.sqlite3")
        with db.transaction() as conn:
            ensure_schema(conn)
        db_mode = stat.S_IMODE(os.stat(db.path).st_mode)
        dir_mode = stat.S_IMODE(os.stat(os.path.dirname(db.path)).st_mode)
        self.assertEqual(db_mode, 0o600)
        self.assertEqual(dir_mode, 0o700)

    def test_sqlite_db_sets_busy_timeout(self) -> None:
        db = SQLiteDB(f"{self.tmpdir.name}/busy/bot.sqlite3")
